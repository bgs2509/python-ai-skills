...
## Completion Gate
Before reporting refresh as complete, you MUST run:
```
grace lint --path <project-root> --profile standard --failOn errors
```
Since refresh edits shared artifacts (knowledge-graph.xml, verification-plan.xml) and may touch MODULE_CONTRACT headers, it is the most likely producer of drift. Exit 0 is the only acceptable success signal. If exit != 0, fix and re-run. Do not report success on a failing lint.

If `grace` CLI is missing, halt and tell the user to install it.
