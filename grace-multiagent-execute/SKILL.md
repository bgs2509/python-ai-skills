...
## Completion Gate
Before the controller reports a wave or phase as complete, it MUST run:
```
grace lint --path <project-root> --profile standard --failOn errors
```
If exit != 0, the wave is NOT complete. Dispatch fix tasks to workers (or fix inline), re-run the lint, and only close the wave when it exits 0. Workers' own reports of success are not sufficient — the controller owns this gate.

If `grace` CLI is missing, halt and tell the user to install it. Never fall back to LLM-based integrity review.
