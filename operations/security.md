...
> Безопасность на всех уровнях — от ввода до логов. Секреты через environment variables (SSoT). Подробнее об управлении секретами — см. `operations/secrets-management.md`.
...
| Правило | Детали |
|---------|--------|
| Секреты только через environment variables | Никаких hardcoded паролей, токенов, API-ключей в коде или git. SSoT — см. `operations/secrets-management.md`. |
...
| Санитизация чувствительных данных в логах | Автоматическая маскировка через structlog процессор (SSoT). См. `development/logging.md`. |
| Валидация на границах системы | Input validation через Pydantic. Не доверяй входным данным (Fail Fast). |
| Параметризованные запросы к БД | SQL injection — blocker. См. `development/database.md`. |
...
| Pre-commit hooks | Автоматическая проверка на утечку секретов. См. `quality/linters.md`. |
...
| 10 | Insufficient Logging | Централизованное логирование всех security-событий. См. `development/logging.md`. |

---

## Docker Security

> См. `operations/docker.md` секция "Security".
