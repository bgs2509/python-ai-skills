...
Скил превращает сырой мультимодальный ввод заказчика (текст, переписка, voice, картинки, схемы) в **один Markdown-файл на русском** — френдли ТЗ, которое разработчик обсуждает с заказчиком ДО старта `do-feature` Discovery.
...
- Заказчик уже дал готовое формальное ТЗ — переходи сразу в `do-feature` Discovery
- Нужна спека для разработчика — используй `do-feature` шаг 2 (Discovery) напрямую
...
После Final скил **не вызывается повторно для этого файла**. Изменения после Final — через `do-feature` Discovery, который читает финальный TZ как вход.
...
2. Подсказать следующий шаг:
   ...
   - Если `status: final` — *«Готово к Discovery. Запусти `do-feature` — он подхватит этот TZ как вход.»*
3. НЕ вызывать автоматически другие скилы. `customer-tz` — worker, не orchestrator.

## Anti-orchestration

Скил не вызывает `do-feature`, не вызывает `brainstorming`, не вызывает `grace-*`. Переходы делает пользователь или внешний orchestrator.
