...
**Rule (choosing WHICH model, not which pool):** the pool rules above say where work belongs; the concrete model, its launch flags, its timeout and its fallback order live in **`~/.claude/model-registry.json`** (repo: `claude-home/model-registry.json`) under five roles — `planner`, `executor`, `researcher` (web research with sources), `batch`, and the reference-only `orchestrator`. Do not restate model names, efforts or timeouts in a skill, a prompt or this file: the registry is their single home, and a copy will drift from it.
