...
### Workflow, GRACE & utility skills (32)
...
- **git & sync:** `git-commit`, `git-branch`, `git-finish` (docs → merge → delete → push, in that order), `project-sync`, `projects-sync`, `beads-sync`
...
> The catalog table above covers only the `_*` quality skills; this grouped list covers the rest. Full inventory = every repo dir with a `SKILL.md` (47 total); `make install-symlinks` links them all.
...
| Commit messages | English | `feat(M-SYNC): add user sender service` |
