...
**CLI is mandatory, not optional.** You MUST run `grace lint --path <project-root> --format json --failOn errors` first and use its JSON report as the authoritative source of findings. If `grace` CLI is not installed (`command -v grace` returns empty), halt immediately and ask the user to install it — do NOT fall back to LLM-based sampling across files. Rationale: LLM sampling is non-deterministic and has been shown to produce a non-converging "integrity spiral" where each run invents new critical findings. The CLI is deterministic across 10k+ files; your role is to interpret its output, not to re-discover findings heuristically.
...
- `grace lint` is the primary source of findings (not a helper). You interpret, group, and prioritize its JSON output; you do not search for findings independently by sampling files. `grace module show` and `grace file show` remain helpers for reading scoped evidence when interpreting a specific finding.
- LLM-based sampling audit is forbidden as a fallback. If CLI is missing or broken, halt and report — never substitute with file-by-file LLM review.
