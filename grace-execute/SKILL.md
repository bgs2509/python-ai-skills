...
## Completion Gate
Before reporting the execution as complete (or marking a phase as done), you MUST run:
```
grace lint --path <project-root> --profile standard --failOn errors
```
If the exit code is non-zero, the skill is NOT complete. Fix the reported errors, re-run the lint, and only report success when it exits 0. Do not declare completion on a failing lint — this is the in-skill killswitch for GRACE integrity drift (paired with the pre-commit hook).

If `grace` CLI is not installed, halt and tell the user to install it. Do not substitute LLM-based review.
